    ...
    return (
      ...
      >
        ...
        <span className={`absolute -bottom-1 left-1/2 w-0 h-0.5 transition-all duration-300 group-hover:w-full group-hover:left-0 ${
          isActive 
            ? 'w-full left-0 ' + (scrolled ? 'bg-primary' : 'bg-accent') 
            ...
            <nav className="hidden md:flex items-center gap-8">
              ...
              {user ? (
                <div className="flex items-center gap-4 pl-2">
                  <Link to="/dashboard" className={`flex items-center gap-3 group ${scrolled ? 'text-slate-800' : 'text-white'}`}>
        ...
        <div className="fixed inset-0 top-20 z-40 bg-slate-900/95 backdrop-blur-md md:hidden animate-fade-in-up">
          <div className="flex flex-col h-full p-6 overflow-y-auto pb-20">
            <nav className="flex flex-col space-y-2">
              <Link to="/" className="flex items-center justify-between text-gray-200 hover:text-accent hover:bg-white/5 p-4 rounded-xl border-b border-white/5 transition-all" onClick={() => setIsMobileMenuOpen(false)}>
                <span className="flex items-center gap-4 font-bold"><Home size={20} /> Início</span> <ChevronRight size={16} />
              </Link>
              <Link to="/locator" className="flex items-center justify-between text-gray-200 hover:text-accent hover:bg-white/5 p-4 rounded-xl border-b border-white/5 transition-all" onClick={() => setIsMobileMenuOpen(false)}>
                <span className="flex items-center gap-4 font-bold"><MapPin size={20} /> Rede Credenciada</span> <ChevronRight size={16} />
              </Link>
              <Link to="/blog" className="flex items-center justify-between text-gray-200 hover:text-accent hover:bg-white/5 p-4 rounded-xl border-b border-white/5 transition-all" onClick={() => setIsMobileMenuOpen(false)}>
            ...
            <div className="mt-8 pt-8 border-t border-white/10">
              {user ? (
                <div className="bg-white/5 rounded-2xl p-6">
                  ...
                  <Link to="/dashboard" className="block w-full text-center bg-primary hover:bg-primary-light text-white py-4 rounded-xl font-bold uppercase tracking-wider mb-3 transition-colors shadow-lg" onClick={() => setIsMobileMenuOpen(false)}>
                    Acessar Meu Painel
                  </Link>
                  <button 
                ...
                <Link to="/login" className="flex items-center justify-center gap-2 w-full text-center bg-white text-slate-900 py-4 rounded-xl font-bold uppercase tracking-widest hover:bg-gray-100 transition-colors shadow-lg" onClick={() => setIsMobileMenuOpen(false)}>
          ...
          <div className="grid md:grid-cols-4 gap-12 border-b border-slate-800 pb-12 mb-12">
            ...
            <div>
              ...
              <ul className="space-y-3 text-sm font-medium">
                <li><Link to="/locator" className="hover:text-white transition-colors block py-1">Rede Credenciada</Link></li>
                <li><Link to="/blog" className="hover:text-white transition-colors block py-1">Blog</Link></li>
                <li><Link to="/login" className="hover:text-white transition-colors block py-1">Portal do Membro</Link></li>
              </ul>
            </div>
            ...
            <div>
              ...
              <ul className="space-y-3 text-sm font-medium">
                ...
                <li><a href="#" className="hover:text-white transition-colors block py-1">Privacidade</a></li>
                <li><a href="#" className="hover:text-white transition-colors block py-1">Contato</a></li>
              </ul>
            </div>
          </div>
